...
function Admin() {
  ...
  const fetchUsers = async (currentUid) => {
    ...
    try {
      const res = await fetchWithAuth(
        getBackendUrl('/admin/users', { userid: currentUid }),
      );
  ...
  const handleAddUser = async (uidToAdd) => {
    ...
    try {
      ...
      const res = await fetchWithAuth(getBackendUrl('/admin/add-user'), {
        method: 'POST',
      ...
      <ContentWrapper>
        ...
        <Section theme={theme}>
          <SectionTitle theme={theme}>
            Whitelisted Users ({allowedUsers.length})
          </SectionTitle>
          <UserList>
            {allowedUsers.map((u) => (
        ...
        <Section theme={theme}>
          <SectionTitle theme={theme}>
            Other Registered Users ({otherUsers.length})
          </SectionTitle>
          {otherUsers.length === 0 ? (
